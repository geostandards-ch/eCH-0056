...
* Directives : elles sont numérotées par norme (p. ex. `WMS-01`). Les directives définissent des exigences obligatoires (voir section xref:conformance[xrefstyle=short]).
* Recommandations : La numérotation se poursuit par spécification pour les recommandations (p. ex. `WMS-10`). Les recommandations sont des spécifications non obligatoires (voir section xref:conformance[xrefstyle=short]).
...
Les directives sont signalées au chapitre xref:guidelines[xrefstyle=short]
 par le mot-clé "*DOIT*", un cadre plus foncé et un fond gris.
...
Les recommandations sont signalées au chapitre xref:guidelines[xrefstyle=short] par le mot-clé "*DEVRAIT*" et un cadre plus clair.
|===
...
|A  |Un géoservice DEVRAIT idéalement utiliser la définition du jeu de matrices de tuiles suisse ci-dessous. 
|===
...
[#appendix-a]
[.appendix nonumber]
== Annexe A – Références normatives et bibliographiques{empty}footnote:[voir issue https://github.com/MediaComem/eCH-0056/issues/20[#20]]
...
include::./appendices/appendix-f-fr.adoc[]